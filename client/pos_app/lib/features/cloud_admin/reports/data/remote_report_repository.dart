...
final class RemoteReportRepository {
  ...
  Future<RemoteReportCatalogs> catalogs() async {
    Future<List<RemoteReportOption>> load(
      String path, {
      bool showCode = false,
    }) async {
      ...
      return raw
          ...
          .map((item) {
            ...
            return RemoteReportOption(
              globalId: id,
              label: showCode && code.isNotEmpty ? '$code · $name' : name,
            );
          })
  ...
  Future<RemoteSummary> summary(RemoteReportFilter filter) async =>
      RemoteSummary.fromJson(
        await _api.get('/api/admin/reports/summary', query: filter.query),
      );
  ...
  Future<RemoteReportTable> table(
    ...
    switch (kind) {
      ...
      case RemoteReportKind.inventory ||
          ...
          RemoteReportKind.paymentMethods:
        break;
    }
    if (kind == RemoteReportKind.products ||
        kind == RemoteReportKind.lowPerformance ||
        kind == RemoteReportKind.inventory ||
        kind == RemoteReportKind.trends) {
      putIfPresent('productGlobalId', productGlobalId);
      putIfPresent('categoryGlobalId', categoryGlobalId);
    }
    if (kind == RemoteReportKind.categories) {
      putIfPresent('categoryGlobalId', categoryGlobalId);
    }
    if (kind == RemoteReportKind.purchases ||
        kind == RemoteReportKind.suppliers) {
      putIfPresent('supplierGlobalId', supplierGlobalId);
    }
    if (kind == RemoteReportKind.sales ||
        kind == RemoteReportKind.users ||
        kind == RemoteReportKind.expenses ||
        kind == RemoteReportKind.cash ||
        kind == RemoteReportKind.cancellations) {
  ...
  RemoteReportTable _tableFrom(
    RemoteReportKind kind,
    Map<String, Object?> json,
  ) {
    List<Map<String, Object?>> rowsFrom(Object? raw) => raw is List
        ? raw.whereType<Map>().map((x) => Map<String, Object?>.from(x)).toList()
        : const [];
    switch (kind) {
      case RemoteReportKind.sales:
      ...
      case RemoteReportKind.lowPerformance:
        return RemoteReportTable(
          ...
          rows: rowsFrom(json['items']),
          note: kind == RemoteReportKind.lowPerformance
      ...
      case RemoteReportKind.expenses:
        return RemoteReportTable(
          title: kind.title,
          columns: const ['group', 'amountCents', 'expenses'],
          rows: rowsFrom(json['breakdown']),
          note:
              'Total: ${json['totalCents'] ?? 0} centavos · ${json['expenses'] ?? 0} gastos',
        );
      ...
      case RemoteReportKind.trends:
        ...
        return RemoteReportTable(
          ...
          rows: rows,
          note: 'Stable = cambio de ingreso entre -5% y +5%. Comparación contra el periodo inmediatamente anterior de igual duración.',
        );
    }
  }
}
