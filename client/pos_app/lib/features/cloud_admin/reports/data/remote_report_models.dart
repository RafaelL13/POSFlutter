...
final class RemoteReportFilter {
  ...
  factory RemoteReportFilter.forPreset(ReportPreset preset, {DateTime? now}) {
    ...
    return switch (preset) {
      ReportPreset.today => RemoteReportFilter(
        fromUtc: today.toUtc(),
        toExclusiveUtc: today.add(const Duration(days: 1)).toUtc(),
        preset: preset,
      ),
      ReportPreset.yesterday => RemoteReportFilter(
        fromUtc: today.subtract(const Duration(days: 1)).toUtc(),
        toExclusiveUtc: today.toUtc(),
        preset: preset,
      ),
      ReportPreset.last7Days => RemoteReportFilter(
        fromUtc: today.subtract(const Duration(days: 6)).toUtc(),
        toExclusiveUtc: today.add(const Duration(days: 1)).toUtc(),
        preset: preset,
      ),
      ReportPreset.thisMonth => RemoteReportFilter(
        fromUtc: DateTime(localNow.year, localNow.month).toUtc(),
        toExclusiveUtc: DateTime(localNow.year, localNow.month + 1).toUtc(),
        preset: preset,
      ),
      ReportPreset.previousMonth => RemoteReportFilter(
        fromUtc: DateTime(localNow.year, localNow.month - 1).toUtc(),
        toExclusiveUtc: DateTime(localNow.year, localNow.month).toUtc(),
        preset: preset,
      ),
      ReportPreset.custom => RemoteReportFilter(
        fromUtc: today.toUtc(),
        toExclusiveUtc: today.add(const Duration(days: 1)).toUtc(),
        preset: preset,
      ),
  ...
  factory RemoteReportFilter.custom(
    DateTime startLocal,
    DateTime endLocalInclusive,
  ) {
    final start = DateTime(startLocal.year, startLocal.month, startLocal.day);
    final endExclusive = DateTime(
      endLocalInclusive.year,
      endLocalInclusive.month,
      endLocalInclusive.day,
    ).add(const Duration(days: 1));
    if (!endExclusive.isAfter(start)) {
      throw ArgumentError('El periodo personalizado es inválido.');
    }
    return RemoteReportFilter(
      fromUtc: start.toUtc(),
      toExclusiveUtc: endExclusive.toUtc(),
  ...
  Map<String, String> get query => {
    ...
    'to': toExclusiveUtc.toIso8601String(),
  };
...
final class ProductTrend {
  const ProductTrend({
    required this.name,
    required this.currentRevenueCents,
