...
class RemoteReportDetailScreen extends StatefulWidget {
  ...
  @override
  State<RemoteReportDetailScreen> createState() =>
      _RemoteReportDetailScreenState();
}
    ...
    body: ListView(
      ...
      children: [
        ...
        _ReportOptions(
          ...
          top: _top,
          onGroupBy: (value) => setState(() {
            _groupBy = value;
            _lastTable = null;
          }),
          onLowMetric: (value) => setState(() {
            _lowMetric = value;
            _lastTable = null;
          }),
          onProductSort: (value) => setState(() {
            _productSort = value;
            _lastTable = null;
          }),
          onTop: (value) => setState(() {
            _top = value;
            _lastTable = null;
          }),
        ),
        ...
        FutureBuilder<RemoteReportCatalogs>(
          ...
          builder: (context, snapshot) {
            if (snapshot.hasError) {
              return const Text(
                'Los filtros de catálogo no están disponibles; el reporte general sigue disponible.',
              );
            }
            ...
            return _DimensionFilters(
              ...
              onProduct: (value) => setState(() {
                _productGlobalId = value;
                _lastTable = null;
              }),
              onCategory: (value) => setState(() {
                _categoryGlobalId = value;
                _lastTable = null;
              }),
              onSupplier: (value) => setState(() {
                _supplierGlobalId = value;
                _lastTable = null;
              }),
              onUser: (value) => setState(() {
                _userGlobalId = value;
                _lastTable = null;
              }),
            );
  ...
  String get _effectiveGroupBy => switch (widget.kind) {
    RemoteReportKind.purchases =>
      _groupBy == 'day' ||
          ...
          ? _groupBy
          : 'supplier',
    RemoteReportKind.expenses =>
      _groupBy == 'day' || _groupBy == 'month' || _groupBy == 'category'
          ? _groupBy
          : 'category',
    _ => _groupBy,
  };

  bool _supportsChart(RemoteReportKind kind) =>
      kind == RemoteReportKind.sales ||
      kind == RemoteReportKind.categories ||
      kind == RemoteReportKind.paymentMethods;
...
class _ReportOptions extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (kind == RemoteReportKind.sales) {
      widgets.add(
        _selector('Agrupar', groupBy, const {
          'day': 'Día',
          'week': 'Semana',
          'month': 'Mes',
        }, onGroupBy),
    ...
    if (kind == RemoteReportKind.lowPerformance) {
      widgets.add(
        _selector('Criterio', lowMetric, const {
          'revenue': 'Menor ingreso',
          'units': 'Menos unidades',
          'no-sales': 'Sin ventas',
    ...
    if (kind == RemoteReportKind.products) {
      widgets.add(
        _selector('Ordenar', productSort, const {
          'revenue': 'Ingreso',
          'units': 'Unidades',
          'profit': 'Utilidad',
          'margin': 'Margen',
    ...
    if (kind == RemoteReportKind.products ||
        kind == RemoteReportKind.lowPerformance ||
        kind == RemoteReportKind.trends) {
      widgets.add(_selectorInt('Top', top, const [20, 50, 100], onTop));
    }
    return widgets.isEmpty
        ? const SizedBox.shrink()
        : Wrap(spacing: 12, runSpacing: 8, children: widgets);
  }
...
class _DimensionFilters extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final productKinds = {
      ...
      RemoteReportKind.lowPerformance,
      RemoteReportKind.inventory,
      RemoteReportKind.trends,
    };
    ...
    final userKinds = {
      RemoteReportKind.sales,
      RemoteReportKind.users,
      RemoteReportKind.expenses,
      RemoteReportKind.cash,
      RemoteReportKind.cancellations,
    };
    if (productKinds.contains(kind)) {
      fields.add(
        _dropdown('Producto', productGlobalId, catalogs.products, onProduct),
      );
    }
    if (categoryKinds.contains(kind)) {
      fields.add(
        _dropdown(
          'Categoría',
          categoryGlobalId,
    ...
    if (supplierKinds.contains(kind)) {
      fields.add(
        _dropdown(
          'Proveedor',
          supplierGlobalId,
    ...
    if (userKinds.contains(kind)) {
      fields.add(_dropdown('Usuario', userGlobalId, catalogs.users, onUser));
    }
...
class _ReportDataTable extends StatelessWidget {
              ...
              .map(
                (row) => DataRow(
                  cells: table.columns
                      .map(
                        (column) =>
                            DataCell(Text(_format(column, row[column]))),
                      )
                      .toList(),
                ),
              )
  ...
  String _format(String column, Object? value) {
    if (column.endsWith('Cents') && value is num) {
      return formatMoney(value.toInt());
    }
...
class _ReportBarChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final valueKey = kind == RemoteReportKind.paymentMethods
        ? 'amountCents'
        : kind == RemoteReportKind.categories
        ? 'revenueCents'
        : 'salesCents';
    ...
    final maxValue = visible.fold<num>(
      0,
      (maxValue, row) =>
          math.max(maxValue, row[valueKey] is num ? row[valueKey] as num : 0),
    );
    return Card(
      child: Padding(
                ...
                child: Row(
                  children: [
                    ...
                    SizedBox(
                      width: 110,
                      child: Text(
                        formatMoney((row[valueKey] as num?)?.toInt() ?? 0),
    ...
    child: Padding(
      ...
      child: Text(
        error is CloudApiException
            ...
            : 'No fue posible cargar el reporte remoto.',
      ),
    ),
  );
}
