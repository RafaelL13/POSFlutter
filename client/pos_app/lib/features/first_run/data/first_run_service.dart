...
final class FirstRunService {
  FirstRunService(this._db, {IdGenerator? ids, PasswordHasher? hasher})
  ...
  Future<bool> configured() async {
    ...
    final r = await db.rawQuery("SELECT COUNT(*) c FROM businesses");
  ...
  Future<void> createBusiness({
    ...
    await _db.criticalTransaction((tx) async {
      final businessGid = _ids.newId(),
          branchGid = _ids.newId(),
          deviceGid = _ids.newId(),
          userGid = _ids.newId();
      ...
      final branchId = await tx.insert('branches', {
        'global_id': branchGid,
      ...
      for (final entry in {
        ...
        'active_user_global_id': userGid,
        'configured': '1',
        'local_session_authenticated': '0',
      }.entries) {
