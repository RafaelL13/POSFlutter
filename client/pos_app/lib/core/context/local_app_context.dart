...
final class LocalAppContext {
  ...
  final String role;
  DeviceMode? get parsedDeviceMode => DeviceMode.tryParse(deviceMode);
  AppRole? get parsedRole => AppRole.tryParse(role);
  bool get isAdminReadOnly => parsedDeviceMode == DeviceMode.adminReadOnly;
  bool get isAdministrator => parsedRole == AppRole.administrator;
  ...
  static Future<LocalAppContext> load(AppDatabase database) async {
    ...
    if (deviceGlobal == null || userGlobal == null) {
      throw StateError('El contexto local no está configurado.');
    }
    const sql =
        'SELECT b.id business_id,b.global_id business_gid,br.id branch_id,br.global_id branch_gid,'
        ...
        ' WHERE d.global_id=? AND u.global_id=?'
        ' AND b.active=1 AND br.active=1 AND d.active=1 AND u.active=1 LIMIT 1';
    final rows = await db.rawQuery(sql, [deviceGlobal, userGlobal]);
    if (rows.isEmpty) {
      throw StateError('No se encontró el contexto activo.');
    }
    final r = rows.first;
    return LocalAppContext(
      businessId: r['business_id'] as int,
      businessGlobalId: r['business_gid'] as String,
      branchId: r['branch_id'] as int,
      branchGlobalId: r['branch_gid'] as String,
      deviceId: r['device_id'] as int,
      deviceGlobalId: r['device_gid'] as String,
      deviceMode: r['device_mode'] as String,
      userId: r['user_id'] as int,
      userGlobalId: r['user_gid'] as String,
